...
export class Neo4jDeleteService implements INodeType {

	// --- Node Description for n8n UI ---
	description: INodeTypeDescription = {
		displayName: 'Neo4j Delete Service', // From TaskInstructions.md
		name: 'neo4jDeleteService', // From TaskInstructions.md
		...
		try {
			...
			for (let i = 0; i < items.length; i++) {
				try {
					...
					const query = 'MATCH (s:Service {service_id: $serviceId}) DETACH DELETE s';
					...
					returnData.push({ json: { success: true, deletedServiceId: serviceId }, pairedItem: { item: i } });
